...
final class DocVisitor extends NodeVisitorAbstract
{
    ...
    public static function create(string $contents): self
    {
        ...
        $context = fn (string $namespace): Context => $contextFactory->createForNamespace($namespace, $contents);

        $docBlockFactory = DocBlockFactory::createInstance();

        $phpdoc = fn (string $doc, Context $context): DocBlock => $docBlockFactory->create($doc, $context);
     ...
     */
    private function resetContext(?Name $name = null): void
    {
        $callable = $this->contextFactory;

        $this->context = $callable($name instanceof Name ? $name->toString() : '');
    }
